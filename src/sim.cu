...
//    cudaFree(d_mass);
    ...
    __global__ void massForcesAndUpdate(CUDA_MASS ** d_mass, int num_masses, double dt, double T, Vec global_acc, CUDA_GLOBAL_CONSTRAINTS c) {
        ...
        if (i < num_masses) {
            ...
            // External Magnet Forces
            Vec external_force = Vec(0,0,0);
            Vec temp; // temporary vector for distance vector
            for (int j = 0; j < num_masses; j++){
                if (i != j) {
                    temp = mass.pos-d_mass[j]->pos;
                    mass.extern_force += -mass.magnetic_force / (1 + temp.norm()) * (temp / temp.norm()); // negative magnet force in the direction of n
                }
            }
            ...
            // reset extern force after use=
            mass.extern_force = Vec(0,0,0);
...
// }
    ...
    void Simulation::segmentMasses(Vec & nr_segments) {
        int key;
    ...
    void Simulation::setEnvDim(int wall_offset) {
        // Limit Simulation Space with walls and create a square playing field
        this->createPlane(titan::Vec(-1, 0, 0), wall_offset, 0.5, 0.3);
        this->createPlane(titan::Vec(1, 0, 0), wall_offset, 0.5, 0.3);
        this->createPlane(titan::Vec(0, -1, 0), wall_offset, 0.5, 0.3);
        this->createPlane(titan::Vec(0, 1, 0), wall_offset, 0.5, 0.3);
        this->env_dim = Vec(wall_offset*2, wall_offset*2, wall_offset*2); // z-direction remains open
    }
